/*
 * Modified by jwzhang, add log for every request
 */
...
public class AccessibleObjectResolver
{
      ...
      CallbackController cbc, ExceptionTransformer exceptionTransformer)
  {
    ...
    try
    {
    	// added by jwzhang, 20111121 	
   	 	// check privilege
//		if (!AuthorizationUtil.isPass(((Method) accessibleObject).getDeclaringClass(), (Method) accessibleObject, SessionProxy
//									.getSecurityContext(((HttpServletRequest) context[0])
//											.getSession()))) {
//		      result = new JSONRPCResult(999, requestId, "No Privilege");
//		      return result;
//		}
      ...
      JSONArray arguments, JSONSerializer serializer) throws UnmarshallException
  {
    ...
    {
      throw new UnmarshallException("arg " + (i + 1) + " could not unmarshall: "+e.getMessage(), e);
    }
