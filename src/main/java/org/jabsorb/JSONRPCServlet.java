...
public class JSONRPCServlet extends HttpServlet {
	...
	/**
	 * a cache for javascript name - javescript
	 */
	protected Map<String, String> javascripts = new HashMap();

	/**
	 * a cache for javascript name - java bean class
	 */
	protected Map<String, Class> javascriptName2Class = new HashMap();
	...
	public void init(ServletConfig config) throws ServletException {
		...
		if (JSONRPCServlet.GZIP_THRESHOLD == -1) {
			log
					.debug("Gzipping is turned OFF.  No attempts will be made to gzip content from this servlet.");
		} else if (JSONRPCServlet.GZIP_THRESHOLD == 0) {
			log
					.debug("All responses will be Gzipped when gzipping results in a smaller response size.");
		} else {
			log
					.debug("Responses over this size will be Gzipped when gzipping results in a smaller response size.");
		}
		initJSONRPCBridge();
	}

	/**
	 * Initialize a GlobalBridge JSONRPCBridge and register all objects and
	 * their security per a config file that read from configFilePath.
	 * 
	 * @param configFilePath
	 * @throws Exception
	 */
	protected void initJSONRPCBridge() {
		JSONRPCBridge json_bridge = JSONRPCBridge.getGlobalBridge();
		Object bean = new AjaxProxy();
		json_bridge.registerObject("AjaxProxy", bean);
		// put this bean's class and name to this map for initializing it.
		javascriptName2Class.put("AjaxProxy", bean.getClass());
	}

	/**
	 * Initialize javascript, one bean one file.
	 * 
	 ...
	 * @return
	 */
	protected String initJavascript(String javascriptName, Class ajaxBeanClass,
			String contextPath, String servletPath) {
		StringBuilder methodNames = new StringBuilder();
		Method[] methods = ajaxBeanClass.getDeclaredMethods();
		for (int i = 0; i < methods.length; i++) {
			if (Modifier.isPublic(methods[i].getModifiers())) {
				methodNames.append("\"");
				methodNames.append(javascriptName);
				methodNames.append(".");
		...
		return "var jsonrpc = new JSONRpcClient([" + methodNames + "], \""
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getPathInfo() != null
			...
			String name = request.getPathInfo().substring(
					request.getPathInfo().lastIndexOf("/") + 1,
					request.getPathInfo().length() - 3);
			String javascript = javascripts.get(name);
			// boolean modified = false;
			if (javascript == null) {
				Class clazz = javascriptName2Class.get(name);
				if (clazz != null) {
					// modified = true;
					javascript = initJavascript(name, clazz, request
							.getContextPath(), request.getServletPath());
					javascripts.put(name, javascript);
					// if (ajaxPerformanceLogger.isDebugEnabled()) {
					// ajaxPerformanceLogger
					// .debug("Initialized javascript: name=" + name
			...
			if (javascript != null) {
				...
				// }
				byte[] bout = javascript.getBytes("UTF-8");
		...
		//cli.setApplicationId(VTLogger.CLI_APP_ID);
		try {
			...
			if (receiveString == null) {
				...
				// save the json-rpc data in a special request attribute, in
				// case a
				// jetty
				// continuation exception (org.mortbay.jetty.RetryRequest) is
				// thrown
				// and this
			...
			try {
				...
				// cli log
				//cli.setFunctionName(method);
				// cli log
				//cli.addData(CliConstants.LABEL_CLIENT_INTO, "aid:" + aid);
				//cli.addData("sid", request.getSession().getId());
				//cli.addData("raddr", request.getRemoteAddr());
			...
			if (JSONRPCServlet.GZIP_THRESHOLD != -1) {
				...
				// but only if the response is large enough to warrant it and if
				// the
			...
			//cli.addData(CliConstants.LABEL_RESULT, String.valueOf(bout.length)
					//+ "B");
	...
	public static void main(String[] args) throws Exception {
		//new JSONRPCServlet().outputJavascript(args[0], args[1], args[2]);
		new JSONRPCServlet().outputJavascript("src/main/webapp/js", "/oml", "/ajax");
		
	}

	/**
	 * Generate js file. This API is used for build
	 * 
	 * @param folderPath
	 * @param contextPath
	 * @param servletPath
	 * @throws Exception
	 */
	public void outputJavascript(String folderPath, String contextPath,
			String servletPath) throws Exception {
		initJSONRPCBridge();
		File folder = new File(folderPath);
		folder.mkdirs();
		for (String name : javascriptName2Class.keySet()) {
			Class clazz = javascriptName2Class.get(name);
			String javascript = initJavascript(name, clazz, contextPath,
					servletPath);
			File file = new File(folder.getAbsolutePath() + "/" + name + ".js");
